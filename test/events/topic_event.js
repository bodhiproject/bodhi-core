...
contract('TopicEvent', function(accounts) {
    ...
    beforeEach(async function() {
        ...
        let tx = await eventFactory.createTopic(...Object.values(TOPIC_PARAMS), { from: OWNER });
    ...
    describe("constructor", async function() {
        ...
        it('can handle a long name using all 10 array slots', async function() {
            let name = ['abcdefghijklmnopqrstuvwxyzabcdef', 'abcdefghijklmnopqrstuvwxyzabcdef',
                ...
                'abcdefghijklmnopqrstuvwxyzabcdef', 'abcdefghijklmnopqrstuvwxyzabcdef',
                'abcdefghijklmnopqrstuvwxyzabcdef', 'abcdefghijklmnopqrstuvwxyzabcdef'];
            testTopic = await TopicEvent.new(0, OWNER, TOPIC_PARAMS._oracle, name, TOPIC_PARAMS._resultNames,
        ...
        it('stops parsing the results when an empty slot is reached', async function() {
            let results = ["first", "second", "", "fourth", "fifth", "sixth", "seventh", "eighth", "ninth", "ten"];
            testTopic = await TopicEvent.new(0, OWNER, TOPIC_PARAMS._oracle, TOPIC_PARAMS._name, results, 
                TOPIC_PARAMS._bettingStartBlock, TOPIC_PARAMS._bettingEndBlock, 
                TOPIC_PARAMS._resultSettingStartBlock, TOPIC_PARAMS._resultSettingEndBlock, 
                addressManager.address);
        ...
        it('can handle using all 10 results', async function() {
        ...
        it('should only set the first 10 results', async function() {
            let results = ["first", "second", "third", "fourth", "fifth", "sixth", "seventh", "eighth", "ninth", 
                "ten", "eleven"];
            testTopic = await TopicEvent.new(0, OWNER, TOPIC_PARAMS._oracle, TOPIC_PARAMS._name, results, 
                TOPIC_PARAMS._bettingStartBlock, TOPIC_PARAMS._bettingEndBlock, 
                TOPIC_PARAMS._resultSettingStartBlock, TOPIC_PARAMS._resultSettingEndBlock, 
        ...
        it('throws if owner address is invalid', async function() {
            try {
                await TopicEvent.new(0, 0, TOPIC_PARAMS._oracle, TOPIC_PARAMS._name, TOPIC_PARAMS._resultNames, 
                    TOPIC_PARAMS._bettingStartBlock, TOPIC_PARAMS._bettingEndBlock, 
                    TOPIC_PARAMS._resultSettingStartBlock, TOPIC_PARAMS._resultSettingEndBlock, 
                    addressManager.address);
                assert.fail();
        ...
        it('throws if oracle address is invalid', async function() {
            try {
                await TopicEvent.new(0, OWNER, 0, TOPIC_PARAMS._name, TOPIC_PARAMS._resultNames, 
                    TOPIC_PARAMS._bettingStartBlock, TOPIC_PARAMS._bettingEndBlock, 
                    TOPIC_PARAMS._resultSettingStartBlock, TOPIC_PARAMS._resultSettingEndBlock, 
                    addressManager.address);
                assert.fail();
        ...
        it('throws if AddressManager address is invalid', async function() {
            try {
                await TopicEvent.new(0, OWNER, TOPIC_PARAMS._centralizedOracle, TOPIC_PARAMS._name, 
        ...
        it('throws if name is empty', async function() {
            try {
                await TopicEvent.new(0, OWNER, TOPIC_PARAMS._centralizedOracle, [], TOPIC_PARAMS._resultNames, 
                    TOPIC_PARAMS._bettingStartBlock, TOPIC_PARAMS._bettingEndBlock, 
                    TOPIC_PARAMS._resultSettingStartBlock, TOPIC_PARAMS._resultSettingEndBlock, 
                    addressManager.address);
                assert.fail();
        ...
        it('throws if eventResults 0 or 1 are empty', async function() {
            try {
                await TopicEvent.new(0, OWNER, TOPIC_PARAMS._centralizedOracle, TOPIC_PARAMS._name, [], 
                    TOPIC_PARAMS._bettingStartBlock, TOPIC_PARAMS._bettingEndBlock, 
                    TOPIC_PARAMS._resultSettingStartBlock, TOPIC_PARAMS._resultSettingEndBlock, 
            ...
            try {
                await TopicEvent.new(0, TOPIC_PARAMS._owner, TOPIC_PARAMS._centralizedOracle, TOPIC_PARAMS._name, 
            ...
            try {
                await TopicEvent.new(0, OWNER, TOPIC_PARAMS._centralizedOracle, TOPIC_PARAMS._name, ["", "second"], 
                    TOPIC_PARAMS._bettingStartBlock, TOPIC_PARAMS._bettingEndBlock, 
                    TOPIC_PARAMS._resultSettingStartBlock, TOPIC_PARAMS._resultSettingEndBlock, 
                    addressManager.address);
                assert.fail();
    ...
    describe("betFromOracle()", async function() {
        it("allows users to bet", async function() {
            ...
            let betBalances = await testTopic.getBetBalances({ from: USER1 });
        ...
        it('throws if receiving from an address that is not the CentralizedOracle contract', async function() {
            ...
            try {
                await testTopic.betFromOracle(USER1, 0, { from: USER1, value: 1 });
                assert.fail();
            } catch(e) {
                SolAssert.assertRevert(e);
            }
    ...
    describe('centralizedOracleSetResult()', async function() {
        ...
        it('sets the result and creates a new CentralizedOracle', async function() {
            ...
            let decentralizedOracle = await testTopic.oracles.call(1);
        ...
        it('throws if allowance is < consensusThreshold', async function() {
            let amount = Utils.getBigNumberWithDecimals(99, BOT_DECIMALS);
    ...
    describe('voteFromOracle()', async function() {
        ...
        it('allows votes from DecentralizedOracles', async function() {
            ...
            let vote1 = Utils.getBigNumberWithDecimals(20, BOT_DECIMALS);
            ...
            let vote2 = Utils.getBigNumberWithDecimals(35, BOT_DECIMALS);
            ...
            let vote3 = Utils.getBigNumberWithDecimals(10, BOT_DECIMALS);
            ...
            let totalVoteBalance = CORACLE_THRESHOLD.add(vote1).add(vote2).add(vote3);
        ...
        it('throws if allowance is less than the amount', async function() {
            let vote = Utils.getBigNumberWithDecimals(10, BOT_DECIMALS);
    ...
    describe('decentralizedOracleSetResult()', async function() {
        ...
        beforeEach(async function() {
            ...
            // Winning vote
            let vote3 = CORACLE_THRESHOLD.add(1);
            await token.approve(testTopic.address, vote3, { from: USER3 });
            assert.equal((await token.allowance(USER3, testTopic.address)).toString(), vote3.toString());
            await decentralizedOracle.voteResult(votingOracle1ResultIndex, vote3, { from: USER3 });
            assert.equal((await testTopic.getVoteBalances({ from: USER3 }))[votingOracle1ResultIndex].toString(), 
                vote3.toString());
    ...
    describe("withdrawWinnings()", async function() {
        ...
        beforeEach(async function() {
            ...
            await centralizedOracle.bet(0, { from: USER1, value: bet1 });
            ...
            await centralizedOracle.bet(1, { from: USER2, value: bet2 });
            ...
            await centralizedOracle.bet(cOracleResult, { from: USER3, value: bet3 });
            totalBetBalance = bet1.add(bet2).add(bet3);
            assert.equal(web3.eth.getBalance(testTopic.address).toNumber(), totalBetBalance.toNumber());
            assert.equal((await testTopic.getBetBalances({ from: USER3 }))[cOracleResult].toString(), 
                bet3.toString());

            bet4 = web3.toBigNumber(1298765432);
            await centralizedOracle.bet(cOracleResult, { from: USER4, value: bet4 });
            totalBetBalance = bet1.add(bet2).add(bet3).add(bet4);
            assert.equal(web3.eth.getBalance(testTopic.address).toNumber(), totalBetBalance.toNumber());
            assert.equal((await testTopic.getBetBalances({ from: USER4 }))[cOracleResult].toString(), 
                bet4.toString());
        ...
        it('transfers the tokens for a single voting round', async function() {
            ...
            let totalVoteBalance = CORACLE_THRESHOLD.add(vote1).add(vote2);
            ...
            // Winners withdraw
            // USER3
            var qtumWon = await testTopic.calculateQtumContributorWinnings({ from: USER3 });
            var botWinningsArray = await testTopic.calculateBotContributorWinnings({ from: USER3 });
            ...
            // USER4
            qtumWon = await testTopic.calculateQtumContributorWinnings({ from: USER4 });
            botWinningsArray = await testTopic.calculateBotContributorWinnings({ from: USER4 });
            ...
            // Losers withdraw
            assert.equal(await testTopic.calculateQtumContributorWinnings({ from: USER1 }), 0);
            botWinningsArray = await testTopic.calculateBotContributorWinnings({ from: USER1 });
            ...
            assert.equal(await testTopic.calculateQtumContributorWinnings({ from: USER2 }), 0);
            botWinningsArray = await testTopic.calculateBotContributorWinnings({ from: USER2 });
            assert.equal(botWinningsArray[0], 0);
            assert.equal(botWinningsArray[1], 0);
        ...
        it('transfers the tokens for a multiple betting/voting rounds', async function() {
            ...
            assert.equal((await testTopic.getVoteBalances({ from: USER1 }))[dOracle1Result].toString(), 
                vote1a.toString());
            ...
            assert.equal((await testTopic.getVoteBalances({ from: USER2 }))[dOracle1Result].toString(), 
                vote2.toString());

            var totalVoteBalance = CORACLE_THRESHOLD.add(vote1a).add(vote2);
            ...
            assert.equal((await testTopic.getVoteBalances({ from: USER3 }))[dOracle2Result].toString(), 
                vote3.toString());
            ...
            assert.equal((await testTopic.getVoteBalances({ from: USER4 }))[dOracle2Result].toString(), 
                vote4.toString());
            ...
            let expectedBotValue = CORACLE_THRESHOLD.add(vote1a).add(vote1b).add(vote2).add(vote3).add(vote4);
    ...
    describe("getBetBalances()", async function() {
        it("returns the bet balances", async function() {
            ...
            let bet0 = Utils.getBigNumberWithDecimals(13, NATIVE_DECIMALS);
            await centralizedOracle.bet(0, { from: ORACLE, value: bet0 });

            let bet1 = Utils.getBigNumberWithDecimals(7, NATIVE_DECIMALS);
            await centralizedOracle.bet(1, { from: ORACLE, value: bet1 });

            let bet2 = Utils.getBigNumberWithDecimals(4, NATIVE_DECIMALS);
    ...
    describe('calculateTokensWon', async function() {
        ...
        describe('single round', async function() {
            beforeEach(async function() {
                ...
                await centralizedOracle.bet(centralizedOracleResult, { from: USER3, value: bet3 });
                totalBetBalance = bet1.add(bet2).add(bet3);
                assert.equal(web3.eth.getBalance(testTopic.address).toNumber(), totalBetBalance.toNumber());

                bet4 = Utils.getBigNumberWithDecimals(12, NATIVE_DECIMALS );
                await centralizedOracle.bet(centralizedOracleResult, { from: USER4, value: bet4 });
                totalBetBalance = bet1.add(bet2).add(bet3).add(bet4);
                assert.equal(web3.eth.getBalance(testTopic.address).toNumber(), totalBetBalance.toNumber());
            ...
            describe('calculateQtumContributorWinnings()', async function() {
                it('returns the QTUM tokens won', async function() {
                    ...
                    assert.equal((await testTopic.calculateQtumContributorWinnings({ from: USER3 })).toString(), 
                        winnings.toString());

                    // USER4: 12 * 31 / 42 = 8 + 10 = 18 blockchain tokens won
                    let bet4MinusCut = bet4.mul(90).div(100);
                    winnings = bet4.mul(losingBlockingTokens).div(winningBlockchainTokens).add(bet4MinusCut);
                    assert.equal((await testTopic.calculateQtumContributorWinnings({ from: USER4 })).toString(), 
                        winnings.toString());
                });

                it('throws if status is not Status:Collection', async function() {
                    try {
                        await testTopic.calculateQtumContributorWinnings({ from: USER3 });
                        assert.fail();
                    } catch(e) {
                        SolAssert.assertRevert(e);
                    }
            ...
            describe('calculateBotContributorWinnings()', async function() {
                it('returns the QTUM and BOT tokens won', async function() {
                    // DecentralizedOracle voting under consensusThreshold
                    let vote1 = Utils.getBigNumberWithDecimals(45, BOT_DECIMALS);
                    await token.approve(testTopic.address, vote1, { from: USER1 });
                    assert.equal((await token.allowance(USER1, testTopic.address)).toString(), vote1.toString());
                    await decentralizedOracle.voteResult(0, vote1, { from: USER1 });
                    assert.equal((await testTopic.getVoteBalances({ from: USER1 }))[0].toString(), vote1.toString());

                    let vote2 = Utils.getBigNumberWithDecimals(54, BOT_DECIMALS);
                    await token.approve(testTopic.address, vote2, { from: USER2 });
                    assert.equal((await token.allowance(USER2, testTopic.address)).toString(), vote2.toString());
                    await decentralizedOracle.voteResult(1, vote2, { from: USER2 });
                    assert.equal((await testTopic.getVoteBalances({ from: USER2 }))[1].toString(), vote2.toString());
                    ...
                    let winningTotalContribution = CORACLE_THRESHOLD.add(bet3).add(bet4);

                    // USER3: 0 BOT votes
                    assert.equal((await testTopic.calculateBotContributorWinnings({ from: USER3 }))[0].toString(), 0);
                    assert.equal((await testTopic.calculateBotContributorWinnings({ from: USER3 }))[1].toString(), 0);

                    // USER4: 0 BOT votes        
                    assert.equal((await testTopic.calculateBotContributorWinnings({ from: USER4 }))[0].toString(), 0);
                    assert.equal((await testTopic.calculateBotContributorWinnings({ from: USER4 }))[1].toString(), 0);

                    // oracle: 100 * 99 / 100 = 99 + 100 = 199 BOT tokens won
                    let oracleBotVote = CORACLE_THRESHOLD;
                    let totalBotContribution = CORACLE_THRESHOLD;
                ...
                it('throws if status is not Status:Collection', async function() {
                    try {
                        await testTopic.calculateBotContributorWinnings({ from: USER3 });
                        assert.fail();
                    } catch(e) {
                        SolAssert.assertRevert(e);
                    }
        ...
        it('returns the QTUM and BOT for multiple rounds', async function() {
            ...
            var threshold = CORACLE_THRESHOLD;
            await blockHeightManager.mineTo(TOPIC_PARAMS._resultSettingStartBlock);
            await token.approve(testTopic.address, threshold, { from: ORACLE });
            await centralizedOracle.setResult(centralizedOracleResult, { from: ORACLE });

            var totalBotValue = CORACLE_THRESHOLD;
            ...
            assert.equal((await testTopic.getVoteBalances({ from: USER1 }))[decentralizedOracle1Result].toString(), 
                vote1a.toString());
            ...
            assert.equal((await testTopic.getVoteBalances({ from: USER2 }))[decentralizedOracle1Result].toString(), 
                vote2a.toString());
            ...
            assert.equal((await testTopic.getVoteBalances({ from: USER3 }))[decentralizedOracle2Result].toString(), 
                vote3a.toString());
            ...
            assert.equal((await testTopic.getVoteBalances({ from: USER4 }))[decentralizedOracle2Result].toString(), 
                vote4a.toString());
            ...
            assert.equal((await testTopic.getVoteBalances({ from: USER5 }))[decentralizedOracle2Result].toString(), 
                vote5a.toString());
            ...
            let winnersBot = Math.floor(CORACLE_THRESHOLD.add(vote3a).add(vote4a).add(vote5a));
            ...
            // USER5 winner
            assert.equal((await testTopic.calculateQtumContributorWinnings({ from: USER5 })).toString(), 0);

            botContribution = vote5a;
            expectedBot = Math.floor(botContribution.mul(losersBot).div(winnersBot).add(botContribution));
            botWinningsArray = await testTopic.calculateBotContributorWinnings({ from: USER5 });
            assert.equal(botWinningsArray[1].toString(), expectedBot.toString());
            ...
            botContribution = CORACLE_THRESHOLD;
            expectedBot = Math.floor(botContribution.mul(losersBot).div(winnersBot).add(botContribution));
            botWinningsArray = await testTopic.calculateBotContributorWinnings({ from: ORACLE });
            assert.equal(botWinningsArray[1].toString(), expectedBot.toString());
            ...
            // USER1 loser
            assert.equal((await testTopic.calculateQtumContributorWinnings({ from: USER1 })).toString(), 0);
            assert.equal((await testTopic.calculateBotContributorWinnings({ from: USER1 }))[0].toString(), 0);
            assert.equal((await testTopic.calculateBotContributorWinnings({ from: USER1 }))[1].toString(), 0);

            // USER2 loser
            assert.equal((await testTopic.calculateQtumContributorWinnings({ from: USER2 })).toString(), 0);
            assert.equal((await testTopic.calculateBotContributorWinnings({ from: USER2 }))[0].toString(), 0);
            assert.equal((await testTopic.calculateBotContributorWinnings({ from: USER2 }))[1].toString(), 0);
        });
    });
});
