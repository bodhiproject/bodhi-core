...
contract EventFactory {
	...
	// Events
	event TopicCreated(address indexed _creator, Topic _topic, bytes32 _name, bytes32[] _resultNames, uint256 _bettingEndBlock);
    
    function createTopic(bytes32 _name, bytes32[] _resultNames, uint256 _bettingEndBlock)
    	public
        returns (Topic tokenAddress)
    {
    	bytes32 topicHash = keccak256(_name, _resultNames, _bettingEndBlock);
